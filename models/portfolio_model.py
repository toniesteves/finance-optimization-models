...
class PortfolioModel(PortfolioGeneric):
    ...
    def getWeights(self):
        ...
        for i in self.N:
            ...
            weights.append(pyo.value(self.model.wl[i] - self.model.ws[i]))
        # end for
    ...
    def sumWeights(self, model):
        return sum(model.wl[j] - model.ws[j] for j in self.N1) + sum(model.wl[j] + model.ws[j] for j in self.N2) == 1
    # end sumWeights

    def minShort(self, model):
        return sum(model.ws[j] for j in self.N1) >= self.settings["minShort"].value
    # end minShort

    def maxShort(self, model):
        return sum(model.ws[j] for j in self.N1) <= self.settings["maxShort"].value
    # end maxShort
    ...
    def linkLong(self, model, j):
        return model.wl[j] <= model.zl[j] * (1 + self.settings["maxShort"].value)
    # end linkLong

    def linkShort(self, model, j):
        return model.ws[j] <= model.zs[j] * self.settings["maxShort"].value
    # end linkShort

    def linkShortFut(self, model, j):
        return model.ws[j] <= model.zs[j] * (1 + self.settings["maxShort"].value)
    # end linkShortFut

    def minFuturesShort(self, model):
        return ((1 + sum(model.ws[j] for j in self.N1)) * self.settings["minFuturesShort"].value
                <= sum(model.ws[j] for j in self.N2))
    # end minFuturesShort

    def maxFuturesShort(self, model):
        return ((1 + sum(model.ws[j] for j in self.N1)) * self.settings["maxFuturesShort"].value
                >= sum(model.ws[j] for j in self.N2))
    # end maxFuturesShort
    ...
    def maxCashPositive(self, model):
        return (model.wl["CASHLEND"] <= (1 + sum(model.ws[j] for j in self.N1))
    ...
    def longLowerBound(self, model, j):
        return model.wl[j] >= model.zl[j] * self.settings["longLowerBound"].value
    # end longLowerBound

    def longUpperBound(self, model, j):
        return model.wl[j] <= model.zl[j] * self.settings["longUpperBound"].value
    # end longUpperBound

    def shortLowerBound(self, model, j):
        return model.ws[j] >= model.zs[j] * self.settings["shortLowerBound"].value
    # end shortLowerBound

    def shortUpperBound(self, model, j):
        return model.ws[j] <= model.zs[j] * self.settings["shortUpperBound"].value
    # end shortUpperBound

    def minExpectedReturn(self, model):
        return sum(self.mu[j] * (model.wl[j] - model.ws[j]) for j in self.N) >= self.target
    # end minExpectedReturn

    def turnover1(self, model, i):
        return model.t[i] >= self.currentPositions[i]  - (model.wl[i] - model.ws[i])
    # end turnover1

    def turnover2(self, model, i):
        return model.t[i] >= (model.wl[i] - model.ws[i]) - self.currentPositions[i]
    # end turnover2

    def turnoverMax(self, model):
        return sum( model.t[i] for i in self.Nrisky) <= self.turnoverLimit
    # end turnoverMax
    ...
    def addAssetLowerBound(self):
        ...
        def assetLowerBound(model, i):
            if bounds[i] >= 0:
                return model.wl[assets[i]] >= bounds[i] * (1 + sum(model.ws[j] for j in self.N1))
            # end if
        ...
        def tagLowerBound(model, i):
            if bounds[i] >= 0:
                return (sum(model.wl[asset] for asset in tagAssets[i]) >= bounds[i]
                        * (1 + sum(model.ws[j] for j in self.N1)))
            # end if
